...
def fixWhiteSpace(BIF_white):
    ...
    while i < len(BIF_white):
        if BIF_white[i] == "\n":  # or a[i] == "}\n":
            ...
            # Add a space after every piece of punctuation. This will make all distinct words separated only by
            # punctuation distinct entries in the list of values when we split a line
        ...
        if lineList[0] == 'variable':
            ...
            while BIF[i] != '}':
                ...
                if lineList[0] == 'table':
                    ...
                    # In the CPD dictionary key, the states of the node are stored first. The second tuple is that of
                    # the parent values
                    theCPD[(temp.getStates(), tuple(lineList[:temp.numParents()]))] = tuple(
                        [float(h) for h in lineList[temp.numParents():]])
                i += 1
            # print theCPD
            temp.setDist(theCPD)
        else:
